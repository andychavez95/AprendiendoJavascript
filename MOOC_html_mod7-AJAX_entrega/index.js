...
const getAPI = async () => {
    ...
    try {
        ...
        alert("No se pudo leer los resultados de la API")
    }
...
const deleteMovie = async (peliculas) => {
    // Completar: Actualizar la información a través de la API
    try {
        let url = localStorage.URL;
        let res = await fetch(url, 
...
const resetMovies = async (peliculas) => {
    // Completar: Actualizar la información a través de la API
    try {
        let url = localStorage.URL;
        let res = await fetch(url, 
...
const createContr = async () => {
    // Completar: controlador que crea una película 
    // nueva en el modelo guardado en myjson
    let newMovie = 
        {
    ...
    //Crea una copia del arreglo de películas actual
    let copia = JSON.parse(JSON.stringify(mis_peliculas));
    copia.push(newMovie);
    
    await addMovie(copia);
...
const deleteContr = async (i) => {
    // Completar:  controlador que actualiza el modelo borrando la 
    // película seleccionada Genera diálogo de confirmación: 
    // botón Aceptar devuelve true, Cancel false
    let borrar = 
        window.confirm(`¿Desea borrar la película ${mis_peliculas[i].titulo}?`);
...
const resetContr = async () => {
    // Completar:  controlador que reinicia el modelo guardado 
    // en myjson con las películas originales
    await resetMovies(mis_peliculas_iniciales);
    indexContr();
}
